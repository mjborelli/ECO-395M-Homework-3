...
# Setting up to evaluate which number of trees provides best improvement of RMSE
RMSE_eval = data.frame(N_Trees = integer(), Mean_RMSE = numeric())
for (i in 1:10){
        cumul_RMSE = 0
        for(j in 1:5) {
                        forest1 = randomForest(Rent ~ size + leasing_rate + stories + age + renovated + class_a + class_b + LEED + Energystar + net + amenities + total_dd_07 + Precipitation + Gas_Costs + Electricity_Costs, mtry=1, nTree=1, data=green_train)
                        yhat_forest_test = predict(forest1, green_test)
                        rmse_forest = mean((green_test$Rent - yhat_forest_test)^2) %>% sqrt
                        cumul_RMSE = cumul_RMSE + rmse_forest
        }
        RMSE_eval[i, 1] = i*10
        RMSE_eval[i, 2] = cumul_RMSE/5
}
ggplot(data = RMSE_eval, aes(x = N_Trees, y = Mean_RMSE)) +
